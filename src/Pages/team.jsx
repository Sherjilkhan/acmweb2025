...
import members from "../assets/Members/member";
...
>>>>>>> 46ab44f6a2674a2584aa276a9cd3e992622d9325
  ...
  const moveHighlight = useCallback(() => {
    ...
    if (currentOption && highlightRef.current && container) {
  ...
  // rest of your code (filteredTeam, MemberCard, JSX)...

  const selected = options[activeIndex];

  // Filter team by year safely
  const filteredTeam = members.filter(
    (member) => (member.Year || "").trim() === selected
  );
  ...
  const noncoreMember = filteredTeam.filter(
    (member) => (member.Type || "").toLowerCase() === "nc"
  );

  // Chapter-wise cores
  const scCores = coreMember.filter(
    (member) => (member.Chapter || "").toLowerCase() === "sc"
  );
  const saiCores = coreMember.filter(
    (member) => (member.Chapter || "").toLowerCase() === "sai"
  );
  ...
  return (
    <div className="Team-container">
      ...
      {wCores.length > 0 && (
        <>
          <h2>Our ACMw Core Team</h2>
          <h4>
            Women in tech are driving innovation and inspiring the future.
          </h4>
          <div className="card-grid">
            {wCores.map((member) => (
              <MemberCard
      ...
      {/* Non-Core Members */}
      {noncoreMember.length > 0 && (
        <>
          <h2>Our Non-Core Team</h2>
          <h4>
            Meet the people that drive RAIT ACM Student Chapter towards success.
          </h4>
          <div className="card-grid">
            {noncoreMember.map((member) => (
              <NcMemberCard
                key={member.Name + member["Position "]}
                member={member}
              />
